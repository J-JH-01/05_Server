...
<%-- c: 자주 사용하는 Java코드가 담겨져 있음(변수선언,if,choose(스위치같은),foreach) 등--%>
...
<body>
	...
	<c:set var="num1" value="10" scope="page" /> <%-- 여는 태그 끝에 / 붙히면 닫는태그 사라짐 --%>
	...
	<c:choose>
		<%--
		 ...
		 --%>
		<c:when test="${empty param.age}"><h4 style="color:red">age 값이 없습니다</h4></c:when>
		<c:when test="${param.age <= 13}"><h4>어린이 입니다</h4></c:when>
		<c:when test="${param.age <= 19}"><h4>청소년 입니다</h4></c:when>
	...
	<ul>
		<c:forEach var="name" items="${nameList}" varStatus="vs">
			...
			<li style="margin-bottom: 10px">
				<ul>
					...
					<%--
						...
						1) ${컬렉션|배열[index]}
						2) ${varStatus.current}
						3) forEach에 var 속성에 작성한 병수명 세팅 후 ${병수명}
					 --%>
			...
			<c:if test="${vs.last}">
				<%-- 반복문의 첫 번째 순회일 때 true --%> <%-- 반복문의 마지막 순회일 때 true --%>
				<li>-------------------종료-------------------</li>
			</c:if>
		</c:forEach>
