...
<head>
<meta charset="UTF-8">
<title>Servlet / 범위별 내장 객체</title>
</head>
<body>
	<h1>Servlet/JSP 범위별 내장 객체</h1>

	<pre>
		Servelt/JSP에는 4종류 범위를 나타낸 내장 객체가 존재한다
		-> 각 종류마다 영향을 끼치는 범위가 달라진다
	
		<h3>1. page : 현재 페이지</h3>
		-> 현재 JSP에서만 사용 가능한 객체(Servlet 사용불가)
		
		<h3>2. request (요청) == HttpServletRequest </h3>
		-> 요청 받은 페이지(Servlet/JSP)와 위임받은페이지(Servlet/JSP) 유지되는 객체
		...
		<h3>4. application (ServletContext라는 이름으로 작성되어있음)</h3>
		...
		** 위에서 아래로 내려올수록 범위도 커짐
		
		<h3>내장 객체 우선 순위 : page > request > session > aplication </h3>
	...
	<pre>
		...
		2) \${key}
			-> 좁은 범위 객체부터 탐색하여 일치하는 key가 있으면 얻어옴	(page에서 찾고 없으면 request 없으면 session 없으면 application)	
	</pre>
